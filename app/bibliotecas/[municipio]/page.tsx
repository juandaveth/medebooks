...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const title = `Bibliotecas en ${name}`;
  const description = `Bibliotecas públicas en ${name}, Área Metropolitana de Medellín, en mapa y lista.`;
  return { title, description, openGraph: { title, description, type: "website" } };
}

export default async function BibliotecasMunicipioPage({ params }: Props) {
  const { municipio } = await params;
